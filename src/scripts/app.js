...
function isNotColliding(uid, eventData, calEvent) { 
  firebase.database().ref('users/' + uid + "/events/").once('value').then(function(data){
    ...
    if (allVals.every(allEventsOk)){ //if no other event overlaps
      ...
      if (currentTime - startM >= 0){ //if event has passed 
        ...
        if (eventNodes.evlocs[0] == eventData.location && eventNodes.evstimes[0] == startM){ //if the event to be created is next event:
...
function geocodeNext(platform, eventData, calEvent, uid, startM, endM, eventNodes, myPosition, currentTime) {
  ...
  }else{var timeGapPost = 0;};
  var timegaps = {timeGapPre, timeGapPost};
  ...
  geocoder.geocode(curEventGeoParams, function(curResult){
    ...
    geocoder.geocode(nxtEventGeoParams, function(nxtResult){
      ...
      if (nxtResult.response != undefined){ // if there's an event after the one to be created:
        ...
        var routeCalcs = {routeToEventParams, routeToNxtEventParams};
      } else { //if there's no event after the one to be created:
        var routeCalcs = {routeToEventParams, 'routeToNxtEventParams': null};
      }
...
function geocodeDef(platform, eventData, calEvent, uid, startM, endM, eventNodes){
  ...
  if (evKplu1_sTime != undefined){ //if event isn't the farthest in time
  ...
  geocoder.geocode(fromEvParams, function(frmResult){
    ...
    geocoder.geocode(toEvParams, function(toResult){
      ...
      geocoder.geocode(nxtEventParams, function(nxtResult){ //obtains the coordinates of all the places needed for event validation
